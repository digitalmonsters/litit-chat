...
} from '/var/task/litit-chat/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/litit-chat/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/litit-chat')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/litit-chat',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/litit-chat/.netlify/dist/run/handlers/server.js')
